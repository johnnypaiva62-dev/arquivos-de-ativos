...
def _descobrir_cnpj_classe(csvs_por_ano: dict, cnpj_fundo: str) -> str:
    """
    Descobre o CNPJ_Fundo_Classe a partir do CNPJ do fundo.
    Busca nos CSVs 'geral' que geralmente têm ambos os campos.
    ...
    for ano in sorted(csvs_por_ano.keys(), reverse=True):
        ...
        for nome_csv, rows in csvs.items():
            if not rows:
                continue
            # Buscar em todos os CSVs, não só geral
            colunas = rows[0].keys()
            col_fundo = None
            col_classe = None
            for c in colunas:
                ...
                if "cnpj" in cl and "classe" in cl:
                    col_classe = c
                elif "cnpj" in cl and "fundo" in cl and "classe" not in cl:
                    col_fundo = c
            
            if not col_classe:
                continue
                
            # Se tem CNPJ_Fundo separado, buscar correspondência direta
            if col_fundo:
                for row in rows:
                    val = re.sub(r"\D", "", str(row.get(col_fundo, "")))
                    if val == cnpj_limpo:
                        return row.get(col_classe, "")
            
            # Se não tem CNPJ_Fundo, checar se o CNPJ_Fundo_Classe == nosso CNPJ
            for row in rows[:50]:
                val = re.sub(r"\D", "", str(row.get(col_classe, "")))
                if val == cnpj_limpo:
                    return cnpj_fundo  # É o mesmo CNPJ
...
async def cvm_contabilidade(
    ...
    cnpj_classe = _descobrir_cnpj_classe(trimestrais, cnpj)
    if not cnpj_classe:
        cnpj_classe = _descobrir_cnpj_classe(anuais, cnpj)
...
async def explorar_trimestral(
    ...
    # Descobrir CNPJ_Fundo_Classe (Resolução CVM 175)
    cnpj_classe = _descobrir_cnpj_classe({ano: csvs}, cnpj)
