...
def _filtrar_por_cnpj(rows: list, cnpj: str, cnpj_classe: str = None) -> list:
    """
    Filtra linhas por CNPJ (busca em todas as colunas que contêm 'cnpj').
    Se cnpj_classe fornecido, também tenta esse CNPJ.
    """
    cnpj_limpo = re.sub(r"\D", "", cnpj)
    cnpjs_busca = {cnpj_limpo}
    if cnpj_classe:
        cnpjs_busca.add(re.sub(r"\D", "", cnpj_classe))
...
def _descobrir_cnpj_classe(csvs_por_ano: dict, cnpj_fundo: str, ticker: str = "") -> str:
    """
    Descobre o CNPJ_Fundo_Classe a partir do CNPJ do fundo ou ticker.
    Necessário para FIIs que migraram para Resolução CVM 175.
    """
    ...
    for ano in sorted(csvs_por_ano.keys(), reverse=True):
        ...
        for nome_csv, rows in csvs.items():
            ...
            # Identificar coluna CNPJ_Fundo_Classe
            col_classe = None
            cols_cnpj_outros = []
            ...
            for c in colunas:
                ...
                if "cnpj" in cl and "classe" in cl:
                    col_classe = c
                elif "cnpj" in cl and "admin" not in cl and "gestor" not in cl and "custodia" not in cl:
                    cols_cnpj_outros.append(c)
            
            if not col_classe:
                continue
            
            # Estratégia 1: buscar CNPJ em colunas CNPJ que NÃO sejam _Classe
            for col in cols_cnpj_outros:
                for row in rows:
                    val = re.sub(r"\D", "", str(row.get(col, "")))
                    if val == cnpj_limpo:
                        cnpj_c = row.get(col_classe, "")
                        if cnpj_c:
                            return cnpj_c
            
            # Estratégia 2: buscar por ticker/ISIN em campos texto
            if ticker_upper:
                cols_texto = [c for c in colunas if any(x in c.lower() for x in 
                    ["codigo", "isin", "negociacao", "ticker", "sigla"])]
                for col in cols_texto:
                    for row in rows:
                        val = str(row.get(col, "")).upper().strip()
                        if ticker_upper in val or ticker_upper.replace("11", "") in val:
                            cnpj_c = row.get(col_classe, "")
                            if cnpj_c:
                                return cnpj_c
            
            # Estratégia 3: buscar CNPJ formatado em qualquer coluna
            cnpj_fmt = f"{cnpj_limpo[:2]}.{cnpj_limpo[2:5]}.{cnpj_limpo[5:8]}/{cnpj_limpo[8:12]}-{cnpj_limpo[12:]}"
            for row in rows[:100]:
                for c in colunas:
                    val_str = str(row.get(c, ""))
                    if cnpj_limpo in re.sub(r"\D", "", val_str) and c != col_classe:
                        cnpj_c = row.get(col_classe, "")
                        if cnpj_c:
                            return cnpj_c
...
def _extrair_contabilidade(rows: list, cnpj: str, cnpj_classe: str = None) -> list:
    """Extrai e estrutura dados contábeis de um CSV de contabilidade."""
    filtrado = _filtrar_por_cnpj(rows, cnpj, cnpj_classe)
    resultados = []
    for row in filtrado:
...
async def cvm_contabilidade(
    ticker: str,
    ano_inicio: int = Query(default=2021, description="Ano inicial"),
    ...
    # Download paralelo trimestrais + anuais
    ...
    cnpj_classe = _descobrir_cnpj_classe(trimestrais, cnpj, ticker)
    if not cnpj_classe:
        cnpj_classe = _descobrir_cnpj_classe(anuais, cnpj, ticker)
    ...
    dre_anual = []
    balanco_ativo_anual = []
    balanco_passivo_anual = []
    
    _debug = {
        "anos_trimestral_baixados": sorted(trimestrais.keys()),
        "anos_anual_baixados": sorted(anuais.keys()),
        "csvs_contabilidade": [],
        "cnpj_buscado": cnpj,
        "cnpj_classe_descoberto": cnpj_classe,
    }

    # Processar trimestrais
    for ano in sorted(trimestrais.keys()):
        ...
        for nome_csv, rows in csvs.items():
            if not rows:
                continue
            nl = nome_csv.lower()
            if "contabilidade" not in nl and "rentabilidade_efetiva" not in nl:
                continue
            
            filtrado = _filtrar_por_cnpj(rows, cnpj, cnpj_classe)
            
            # Debug
            colunas_cnpj = [k for k in rows[0].keys() if "cnpj" in k.lower()] if rows else []
            cnpj_exemplos = []
            if rows and colunas_cnpj:
                for col in colunas_cnpj:
                    vals = set(r.get(col, "")[:25] for r in rows[:5])
                    cnpj_exemplos.append({"col": col, "exemplos": list(vals)})
            
            _debug["csvs_contabilidade"].append({
                "csv": nome_csv,
                "ano": ano,
                "total_rows": len(rows),
                "filtrado_ticker": len(filtrado),
                "colunas_cnpj": colunas_cnpj,
                "cnpj_exemplos": cnpj_exemplos,
                "colunas": list(rows[0].keys())[:15] if rows else [],
            })

            dados = _extrair_contabilidade(rows, cnpj, cnpj_classe)
            if not dados:
                continue

            if "dre" in nl or "rentabilidade_efetiva" in nl:
                for d in dados:
                    d["_fonte"] = "trimestral"
                    d["_csv"] = nome_csv
    ...
    for ano in sorted(anuais.keys()):
        ...
        for nome_csv, rows in csvs.items():
            ...
            if "dre" in nl:
                for d in dados:
                    d["_fonte"] = "anual"
                dre_anual.extend(dados)
            elif "ativo" in nl and "passivo" not in nl:
                for d in dados:
                    d["_fonte"] = "anual"
                balanco_ativo_anual.extend(dados)
            elif "passivo" in nl:
                for d in dados:
                    d["_fonte"] = "anual"
                balanco_passivo_anual.extend(dados)

    # Ordenar por data
    for lista in [dre_trimestral, balanco_ativo_trimestral, balanco_passivo_trimestral,
                  dre_anual, balanco_ativo_anual, balanco_passivo_anual]:
    ...
    return {
        ...
        "dre": {
            "trimestral": dre_trimestral,
            "anual": dre_anual,
        },
        "balanco": {
            "ativo": {
                "trimestral": balanco_ativo_trimestral,
                "anual": balanco_ativo_anual,
            },
            "passivo": {
                "trimestral": balanco_passivo_trimestral,
                "anual": balanco_passivo_anual,
            },
        },
        "resumo": {
            "trimestres_dre": len(dre_trimestral),
            "trimestres_balanco": len(balanco_ativo_trimestral),
            "anos_dre": len(dre_anual),
            "anos_balanco": len(balanco_ativo_anual),
        },
        "_debug": _debug,
        "consultado_em": datetime.now().isoformat(),
    }
...
async def cvm_imoveis(
    ticker: str,
    ano_inicio: int = Query(default=2022, description="Ano inicial"),
    ano_fim: int = Query(default=None, description="Ano final (default: atual)"),
):
    """
...
async def explorar_trimestral(
    ...
    # Descobrir CNPJ_Fundo_Classe (Resolução CVM 175)
    cnpj_classe = _descobrir_cnpj_classe({ano: csvs}, cnpj, ticker)
