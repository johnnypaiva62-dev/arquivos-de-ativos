...
# ─────────────────────────────────────────────────────────────
# Acompanhamento de Carteira (Anual valor contábil + Trimestral imóvel)
# ─────────────────────────────────────────────────────────────

@app.get("/api/cvm/carteira/{ticker}")
async def cvm_carteira(
    ticker: str,
    ano_inicio: int = Query(default=2016, description="Ano inicial"),
    ano_fim: int = Query(default=None, description="Ano final"),
):
    """
    Acompanhamento da carteira de ativos de um FII.

    Fontes:
    - Anual: inf_anual_fii_ativo_valor_contabil (valor contábil, valor justo, % valorização)
    - Trimestral: inf_trimestral_fii_imovel (endereço, área, vacância, inadimplência, receita)
    """
    ticker = ticker.upper().strip()
    tipo = detectar_tipo_ativo(ticker)
    if tipo != "fii":
    ...
    # ── Helper: encontrar coluna CNPJ e filtrar ──
    def _filtrar_csv(rows):
        if not rows:
            return []
        col_cnpj = None
        ...
        if not col_cnpj:
            return []
        return [row for row in rows if re.sub(r"\D", "", str(row.get(col_cnpj, "") or "")) == cnpj_limpo]

    # ── 1) Anual: valor contábil dos ativos ──
    # Colunas: CNPJ_Fundo_Classe, Data_Referencia, Versao, Nome_Ativo, Valor, Valor_Justo, Percentual_Valorizacao_Desvalorizacao
    ativos_valor = []
    for ano in sorted(anuais.keys()):
        csvs = anuais[ano]
        for nome_csv, rows in csvs.items():
            if "ativo_valor_contabil" not in nome_csv.lower():
                continue
            for row in _filtrar_csv(rows):
                nome = (row.get("Nome_Ativo") or "").strip()
                if not nome:
                    continue
                valor_raw = row.get("Valor", "")
                pct_raw = row.get("Percentual_Valorizacao_Desvalorizacao", "")
                valor_justo_raw = row.get("Valor_Justo", "")
                ativos_valor.append({
                    "data_referencia": row.get("Data_Referencia", ""),
                    "_ano": ano,
                    "nome_ativo": nome,
                    "valor_contabil": _extrair_float_safe(valor_raw),
                    "valor_justo": valor_justo_raw.strip() if isinstance(valor_justo_raw, str) else valor_justo_raw,
                    "percentual_valorizacao": _extrair_float_safe(pct_raw),
                })

    # ── 2) Trimestral: imóveis com operacional ──
    # Colunas: CNPJ_Fundo_Classe, Data_Referencia, Versao, Classe, Nome_Imovel, Endereco, Area,
    #          Numero_Unidades, Outras_Caracteristicas_Relevantes,
    #          Percentual_Vacancia, Percentual_Inadimplencia, Percentual_Receitas_FII,
    #          Percentual_Locado, Percentual_Vendido,
    #          Percentual_Conclusao_Obras_Realizado/Previsto, Custo_Construcao_Realizado/Previsto,
    ...
    for ano in sorted(trimestrais.keys()):
        ...
        for nome_csv, rows in csvs.items():
            nl = nome_csv.lower()
            # Apenas inf_trimestral_fii_imovel (não renda_acabados, construcao, venda)
            if "fii_imovel" not in nl:
                continue
            if "renda" in nl or "construcao" in nl or "venda" in nl:
                continue
            for row in _filtrar_csv(rows):
                nome = (row.get("Nome_Imovel") or "").strip()
                if not nome:
                    continue
                imoveis_trimestral.append({
                    "data_referencia": row.get("Data_Referencia", ""),
                    "_ano": ano,
                    "nome_imovel": nome,
                    "classe": (row.get("Classe") or "").strip(),
                    "endereco": (row.get("Endereco") or "").strip(),
                    "area": _extrair_float_safe(row.get("Area", "")),
                    "numero_unidades": _extrair_float_safe(row.get("Numero_Unidades", "")),
                    "caracteristicas": (row.get("Outras_Caracteristicas_Relevantes") or "").strip(),
                    "percentual_vacancia": _extrair_float_safe(row.get("Percentual_Vacancia", "")),
                    "percentual_inadimplencia": _extrair_float_safe(row.get("Percentual_Inadimplencia", "")),
                    "percentual_receitas_fii": _extrair_float_safe(row.get("Percentual_Receitas_FII", "")),
                    "percentual_locado": _extrair_float_safe(row.get("Percentual_Locado", "")),
                    "percentual_vendido": _extrair_float_safe(row.get("Percentual_Vendido", "")),
                    "percentual_conclusao_obras": _extrair_float_safe(row.get("Percentual_Conclusao_Obras_Realizado", "")),
                    "custo_construcao_realizado": _extrair_float_safe(row.get("Custo_Construcao_Realizado", "")),
                    "custo_construcao_previsto": _extrair_float_safe(row.get("Custo_Construcao_Previsto", "")),
                    "percentual_imovel_total_investido": _extrair_float_safe(row.get("Percentual_Imovel_Total_Investido", "")),
                })

    # Ordenar
    ativos_valor.sort(key=lambda x: (x.get("data_referencia", ""), x.get("nome_ativo", "")))
    imoveis_trimestral.sort(key=lambda x: (x.get("data_referencia", ""), x.get("nome_imovel", "")))

    # ── Consolidar: snapshot mais recente por ativo ──
    # Anual: último registro por nome_ativo
    snapshot_valor = {}
    for a in ativos_valor:
        snapshot_valor[a["nome_ativo"]] = a

    # Trimestral: último registro por nome_imovel
    snapshot_imovel = {}
    for i in imoveis_trimestral:
        snapshot_imovel[i["nome_imovel"]] = i

    # ── Merge: combinar valor contábil + dados operacionais ──
    carteira_atual = []
    nomes_processados = set()

    for nome, ativo in snapshot_valor.items():
        # Tentar match com imóvel trimestral (nome pode ser ligeiramente diferente)
        imovel = snapshot_imovel.get(nome)
        if not imovel:
            # Tentar match parcial
            for ni, im in snapshot_imovel.items():
                if nome.lower() in ni.lower() or ni.lower() in nome.lower():
                    imovel = im
                    break

        item = {
            "nome": nome,
            "valor_contabil": ativo["valor_contabil"],
            "percentual_valorizacao": ativo["percentual_valorizacao"],
            "data_valor": ativo["data_referencia"],
        }
        if imovel:
            item.update({
                "endereco": imovel["endereco"],
                "area": imovel["area"],
                "classe": imovel["classe"],
                "vacancia": imovel["percentual_vacancia"],
                "inadimplencia": imovel["percentual_inadimplencia"],
                "percentual_receitas": imovel["percentual_receitas_fii"],
                "percentual_locado": imovel["percentual_locado"],
                "data_operacional": imovel["data_referencia"],
            })
            nomes_processados.add(imovel["nome_imovel"])

        carteira_atual.append(item)

    # Adicionar imóveis que só existem no trimestral (sem valor contábil)
    for nome, imovel in snapshot_imovel.items():
    ...
    # Ordenar carteira por valor (maior primeiro)
    carteira_atual.sort(key=lambda x: x.get("valor_contabil", 0) or 0, reverse=True)

    # Resumo
    total_valor = sum(a.get("valor_contabil", 0) or 0 for a in carteira_atual)
    total_ativos = len(carteira_atual)
    vacancia_media = 0
    imoveis_com_vacancia = [a for a in carteira_atual if a.get("vacancia") is not None and a.get("vacancia", 0) > 0 or a.get("vacancia") == 0]
    if imoveis_com_vacancia:
        vacancia_media = sum(a.get("vacancia", 0) or 0 for a in imoveis_com_vacancia) / len(imoveis_com_vacancia)
    ...
    return {
        ...
        "historico_valor": ativos_valor,
        "historico_imoveis": imoveis_trimestral,
        "resumo": {
            "total_ativos": total_ativos,
            "valor_total_carteira": total_valor,
            "vacancia_media": vacancia_media,
            "total_registros_valor": len(ativos_valor),
            "total_registros_imoveis": len(imoveis_trimestral),
        },
        "consultado_em": datetime.now().isoformat(),
    }
