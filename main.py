...
async def cvm_primeiro_informe(ticker: str):
    ...
    for ano in range(2016, ano_atual + 1):
        ...
        # Buscar no CSV "geral" que tem os dados financeiros principais
        for nome_csv, rows in csvs.items():
            if not rows:
                continue
            ...
            for k, v in primeiro.items():
                if v and v != "" and v != "None" and v != "0" and v != "0.00":
                    campos_relevantes[k] = v

            # Tentar calcular VP/cota
            pl = None
            cotas = None
            for k, v in primeiro.items():
                kl = k.lower()
                if ("patrim" in kl and "liq" in kl) or k in ("VL_PATRIM_LIQ", "Patrimonio_Liquido"):
                    try:
                        pl = float(str(v).replace(",", "."))
                    except:
                        pass
                if "qt_cota" in kl or "numero_cotista" in kl or k in ("NR_COTST",):
                    pass  # cotistas, não cotas
                if "cota" in kl and ("qt" in kl or "quant" in kl or "numero" in kl):
                    try:
                        cotas = float(str(v).replace(",", "."))
                    except:
                        pass
...
async def cvm_historico_vp(ticker: str):
    ...
    for ano in range(2016, ano_atual + 1):
        ...
        # Pegar o CSV "geral" que tem PL e cotas
        for nome_csv, rows in csvs.items():
            # Focar nos CSVs que tipicamente têm PL
            nome_lower = nome_csv.lower()
            if "complemento" in nome_lower or "ativo_passivo" in nome_lower:
                continue  # pular CSVs que não têm PL
            ...
            for row in filtrado:
                ...
                for k, v in row.items():
                    if not v or v == "0" or v == "0.00":
                        continue
                    kl = k.lower()
                    if ("patrim" in kl and "liq" in kl) or k in ("VL_PATRIM_LIQ",):
                        try:
                            pl = float(str(v).replace(",", "."))
                        except:
                            pass
                    if ("qt_cota" in kl or ("cota" in kl and ("qt" in kl or "quant" in kl))):
                        try:
                            val = float(str(v).replace(",", "."))
                            if val > 100:  # provavelmente é quantidade de cotas, não cotistas
                                cotas = val
                        except:
                            pass
            ...
            # Só usar o primeiro CSV que encontrou dados (evitar duplicatas)
            if pontos and any(p["ano"] == ano for p in pontos):
                break
