...
async def cvm_transacoes(
    ...
    """
    Retorna histórico de aquisições e alienações de ativos de um FII.

    Fontes:
    - Informe Trimestral: aquisição/alienação de imóveis e terrenos
    - Informe Anual: ativos adquiridos e vendidos no exercício

    Dados: endereço, valor, data, área, tipo de ativo, contraparte.
    """
    ticker = ticker.upper().strip()
    tipo = detectar_tipo_ativo(ticker)
    if tipo != "fii":
    ...
    # Descobrir CNPJ_Fundo_Classe (Resolução CVM 175)
    cnpjs_classe = _descobrir_cnpj_classe(trimestrais, cnpj, ticker)
    cnpjs_classe_anual = _descobrir_cnpj_classe(anuais, cnpj, ticker)
    # Unificar
    todos_cnpjs = list(set(cnpjs_classe + cnpjs_classe_anual))

    aquisicoes_imoveis = []
    alienacoes_imoveis = []
    aquisicoes_terrenos = []
    alienacoes_terrenos = []
    ativos_adquiridos_anual = []
    ativos_vendidos_anual = []
    ...
    for ano in sorted(trimestrais.keys()):
        ...
        for nome_csv, rows in csvs.items():
            ...
            # Só processar CSVs de aquisição/alienação
            eh_transacao = False
            tipo_transacao = ""
            tipo_ativo = ""

            if "aquisicao" in nl and "imovel" in nl:
                eh_transacao = True
                tipo_transacao = "aquisicao"
                tipo_ativo = "imovel"
            elif "alienacao" in nl and "imovel" in nl:
                eh_transacao = True
                tipo_transacao = "alienacao"
                tipo_ativo = "imovel"
            elif "aquisicao" in nl and "terreno" in nl:
                eh_transacao = True
                tipo_transacao = "aquisicao"
                tipo_ativo = "terreno"
            elif "alienacao" in nl and "terreno" in nl:
                eh_transacao = True
                tipo_transacao = "alienacao"
                tipo_ativo = "terreno"
            ...
            filtrado = _filtrar_por_cnpj(rows, cnpj, todos_cnpjs)
            for row in filtrado:
                dt = row.get("Data_Referencia") or row.get("DT_COMPTC") or ""
                registro = {
                    "data_referencia": dt,
                ...
                if len(registro) > 5:  # Tem dados além dos campos base
                    if tipo_transacao == "aquisicao" and tipo_ativo == "imovel":
                        aquisicoes_imoveis.append(registro)
                    elif tipo_transacao == "alienacao" and tipo_ativo == "imovel":
                        alienacoes_imoveis.append(registro)
                    elif tipo_transacao == "aquisicao" and tipo_ativo == "terreno":
                        aquisicoes_terrenos.append(registro)
                    elif tipo_transacao == "alienacao" and tipo_ativo == "terreno":
                        alienacoes_terrenos.append(registro)
    ...
    for ano in sorted(anuais.keys()):
        ...
        for nome_csv, rows in csvs.items():
            ...
            if "ativo_adquirido" in nl or ("adquirido" in nl and "ativo" in nl):
                eh_transacao = True
                tipo_transacao = "aquisicao"
            elif "ativo_vendido" in nl or ("vendido" in nl and "ativo" in nl):
                eh_transacao = True
                tipo_transacao = "alienacao"
            ...
            for row in filtrado:
                ...
                if len(registro) > 5:
                    if tipo_transacao == "aquisicao":
                        ativos_adquiridos_anual.append(registro)
                    else:
                        ativos_vendidos_anual.append(registro)

    # Ordenar tudo por data
    for lista in [aquisicoes_imoveis, alienacoes_imoveis, aquisicoes_terrenos,
                  alienacoes_terrenos, ativos_adquiridos_anual, ativos_vendidos_anual]:
        lista.sort(key=lambda x: x.get("data_referencia", ""))

    # Consolidar em timeline única
    todas_transacoes = []
    for item in aquisicoes_imoveis + aquisicoes_terrenos + ativos_adquiridos_anual:
        item["_direcao"] = "compra"
        todas_transacoes.append(item)
    for item in alienacoes_imoveis + alienacoes_terrenos + ativos_vendidos_anual:
        item["_direcao"] = "venda"
        todas_transacoes.append(item)

    todas_transacoes.sort(key=lambda x: x.get("data_referencia", ""), reverse=True)

    # Calcular totais
    total_compras = sum(
        item.get("Valor_Aquisicao", 0) or item.get("Valor_Compra", 0) or
        item.get("Custo_Aquisicao", 0) or 0
        for item in todas_transacoes if item.get("_direcao") == "compra"
    )
    total_vendas = sum(
        item.get("Valor_Alienacao", 0) or item.get("Valor_Venda", 0) or 0
        for item in todas_transacoes if item.get("_direcao") == "venda"
    )
    ...
    return {
        ...
        "nome": dados_fii.get("razao_social", ""),
        "periodo": f"{ano_inicio}-{ano_fim}",
        "timeline": todas_transacoes,
        "aquisicoes": {
            "imoveis": aquisicoes_imoveis,
            "terrenos": aquisicoes_terrenos,
            "ativos_anual": ativos_adquiridos_anual,
        },
        "alienacoes": {
            "imoveis": alienacoes_imoveis,
            "terrenos": alienacoes_terrenos,
            "ativos_anual": ativos_vendidos_anual,
        },
        "resumo": {
            "total_transacoes": len(todas_transacoes),
            "total_aquisicoes": len(aquisicoes_imoveis) + len(aquisicoes_terrenos) + len(ativos_adquiridos_anual),
            "total_alienacoes": len(alienacoes_imoveis) + len(alienacoes_terrenos) + len(ativos_vendidos_anual),
