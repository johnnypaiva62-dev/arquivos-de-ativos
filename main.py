...
async def cvm_transacoes(
    ...
    Fonte: Informe Anual CVM (ativos adquiridos e vendidos no exercício).
    """
    ticker = ticker.upper().strip()
    tipo = detectar_tipo_ativo(ticker)
    if tipo != "fii":
    ...
    try:
        # Download paralelo anuais
        anuais = _download_anual_parallel(ano_inicio, ano_fim)

        if not anuais:
            return {
                "ticker": ticker, "cnpj": cnpj,
                "nome": dados_fii.get("razao_social", ""),
                "periodo": f"{ano_inicio}-{ano_fim}",
                "timeline": [], "adquiridos": [], "vendidos": [],
                "resumo": {"total_transacoes": 0, "total_aquisicoes": 0, "total_alienacoes": 0,
                           "volume_compras": 0, "volume_vendas": 0, "saldo_liquido": 0},
                "consultado_em": datetime.now().isoformat(),
            }

        # Descobrir CNPJ_Fundo_Classe
        cnpjs_classe = _descobrir_cnpj_classe(anuais, cnpj, ticker)
        ...
        def _extrair_valor(row, campos):
            """Extrai o primeiro valor numérico > 0 dos campos listados."""
            for c in campos:
                v = row.get(c)
            ...
            for k, v in row.items():
                kl = k.lower()
                if ("valor" in kl or "custo" in kl or "preco" in kl) and isinstance(v, (int, float)) and v > 0:
                    return v
            return 0
        ...
        for ano in sorted(anuais.keys()):
            ...
            for nome_csv, rows in csvs.items():
                ...
                # Apenas CSVs de ativo_adquirido e ativo_vendido
                if "ativo_adquirido" in nl:
                    tipo_transacao = "aquisicao"
                elif "ativo_vendido" in nl:
                ...
                # Filtrar por CNPJ
                cnpj_limpo = re.sub(r"\D", "", cnpj)
                cnpjs_set = {cnpj_limpo}
                for c in (cnpjs_classe or []):
                    cl = re.sub(r"\D", "", c) if c else ""
                    if cl:
                        cnpjs_set.add(cl)

                # Descobrir qual coluna tem CNPJ (uma vez por CSV)
                col_cnpj = None
                if rows:
                    for col_name in rows[0].keys():
                        # Normalizar: strip, BOM, lowercase
                        cn = col_name.strip().replace("\ufeff", "").replace("\u200b", "").lower()
                        cn = re.sub(r"[^\x20-\x7e]", "", cn)  # Remover qualquer char não-ASCII
                        if "cnpj" in cn:
                            col_cnpj = col_name  # Usar o nome ORIGINAL (com BOM se tiver)
                            break

                filtrado = []
                if col_cnpj:
                    for row in rows:
                        val = str(row.get(col_cnpj, "") or "")
                        val_limpo = re.sub(r"\D", "", val)
                        if val_limpo in cnpjs_set:
                            filtrado.append(row)
                else:
                    # Sem coluna CNPJ identificada — fallback: checar todas as colunas
                    # mas só valores que parecem CNPJ (exatamente 14 dígitos com / ou .)
                    for row in rows:
                        for col_val in row.values():
                            val_str = str(col_val or "").strip()
                            if ("/" in val_str and "." in val_str) or \
                               (len(val_str) == 18 and val_str[2] == "." and val_str[6] == "."):
                                val_limpo = re.sub(r"\D", "", val_str)
                                if val_limpo in cnpjs_set:
                                    filtrado.append(row)
                                    break
                ...
                _debug_csvs.append({
                    ...
                    "ano": ano,
                    "total_rows": len(rows),
                    "col_cnpj_encontrada": col_cnpj,
                    "col_cnpj_repr": repr(col_cnpj) if col_cnpj else None,
                    "todas_colunas": list(rows[0].keys()) if rows else [],
                    "primeira_col_repr": repr(list(rows[0].keys())[0]) if rows else None,
                    "filtrado": len(filtrado),
                    "cnpjs_buscados": list(cnpjs_set),
                    "amostra_cnpj_csv": re.sub(r"\D", "", str(rows[0].get(col_cnpj, ""))) if col_cnpj and rows else "N/A",
                })
                ...
                for row in filtrado:
                    ...
                    if len(registro) > 4:
                        if tipo_transacao == "aquisicao":
                            registro["_direcao"] = "compra"
                            registro["_valor"] = _extrair_valor(registro, 
                                ["Valor_Aquisicao", "Custo_Aquisicao", "Valor_Compra", "Valor_Total"])
                            adquiridos.append(registro)
                        else:
                            registro["_direcao"] = "venda"
                            registro["_valor"] = _extrair_valor(registro,
                                ["Valor_Alienacao", "Valor_Venda", "Preco_Venda", "Valor_Total"])
        ...
        timeline = adquiridos + vendidos
        timeline.sort(key=lambda x: x.get("data_referencia", ""), reverse=True)
        ...
        return {
            ...
            "_debug": {
                ...
                "anos_baixados": sorted(anuais.keys()),
                "csvs_processados": _debug_csvs,
            },
            "consultado_em": datetime.now().isoformat(),
        }
    except Exception as e:
        return {
            "ticker": ticker,
            "cnpj": cnpj,
            "erro": f"Erro ao processar transações: {type(e).__name__}: {str(e)}",
            "timeline": [], "adquiridos": [], "vendidos": [],
            "resumo": {"total_transacoes": 0, "total_aquisicoes": 0, "total_alienacoes": 0,
                       "volume_compras": 0, "volume_vendas": 0, "saldo_liquido": 0},
            "consultado_em": datetime.now().isoformat(),
