...
def _filtrar_por_cnpj(rows: list, cnpj: str) -> list:
    """Filtra linhas por CNPJ (tenta várias colunas possíveis)."""
    cnpj_limpo = re.sub(r"\D", "", cnpj)
    resultado = []
    for row in rows:
        # A CVM usa nomes diferentes dependendo do ano:
        # 2021-2024: CNPJ_FUNDO
        # 2025+: CNPJ_FUNDO_CLASSE
        row_cnpj = ""
        for col in ("CNPJ_FUNDO", "CNPJ_FUNDO_CLASSE", "CNPJ_FUNDO_COTA", "cnpj_fundo", "cnpj"):
...
async def cvm_informe_mensal(
    ...
    # Determinar anos para tentar (atual + anterior)
    ano_atual = datetime.now().year
    anos_tentar = [ano] if ano else [ano_atual, ano_atual - 1]
    ...
    for a in anos_tentar:
        result = _baixar_csv_cvm_fii(a)
        if "erro" not in result and result:
            csvs = result
            ano_usado = a
            break
        else:
            erro_download = result.get("erro", "Sem dados")

    if not csvs:
        return {
            "ticker": ticker,
            "cnpj": cnpj,
            "erro": erro_download or "Não foi possível baixar dados da CVM",
            "debug": {
    ...
    # Debug: listar CSVs encontrados e suas colunas
    debug_csvs = {}
    for nome, rows in csvs.items():
        colunas = list(rows[0].keys()) if rows else []
        # Verificar se há registros com o CNPJ buscado
        matches = _filtrar_por_cnpj(rows, cnpj)
        debug_csvs[nome] = {
            "total_linhas": len(rows),
            "colunas_cnpj": [c for c in colunas if "CNPJ" in c.upper() or "cnpj" in c],
            "matches_cnpj": len(matches),
            "amostra_cnpj": rows[0].get("CNPJ_FUNDO", rows[0].get("CNPJ_FUNDO_CLASSE", "N/A")) if rows else "vazio",
        }
    ...
    resultado = {
        ...
        "ano": ano_usado,
        "fonte": f"https://dados.cvm.gov.br/dados/FII/DOC/INF_MENSAL/DADOS/inf_mensal_fii_{ano_usado}.zip",
        "dados": {},
    ...
    for nome_csv, rows in csvs.items():
        ...
        nome_curto = nome_csv.replace(f"_{ano_usado}", "").replace("inf_mensal_fii_", "").replace(".csv", "")
