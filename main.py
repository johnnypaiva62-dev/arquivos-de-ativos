...
def descobrir_dados_fii(ticker: str) -> dict:
    ...
    Ambos são necessários para buscar no FNET de forma confiável.
    """
    ...
    idx_razao = None
    idx_cod = None
    idx_sit = None  # situação (para filtrar fundos ativos)

    for i, h in enumerate(header_upper):
        if "CNPJ" in h and idx_cnpj is None:
            idx_cnpj = i
        if "RAZAO" in h or "DENOM" in h or "NOME" in h:
            if idx_razao is None:
                idx_razao = i
        if "COD" in h and "CVM" in h:
            idx_cod = i
        if "SIT" in h:
            idx_sit = i
    ...
    for linha in lines[1:]:
        if not linha.strip():
            continue
        # Primeiro: buscar pelo ticker no texto da linha
        ...
        # Extrair CNPJ
        if idx_cnpj is not None and idx_cnpj < len(campos):
            cnpj_val = campos[idx_cnpj].strip()
            if re.match(r"\d{2}\.\d{3}\.\d{3}/\d{4}-\d{2}", cnpj_val):
                result["cnpj"] = cnpj_val

        # Fallback: buscar CNPJ em qualquer campo da linha
        if not result["cnpj"]:
            match = re.search(r"(\d{2}\.\d{3}\.\d{3}/\d{4}-\d{2})", linha)
            if match:
                result["cnpj"] = match.group(1)

        # Extrair Razão Social / Denominação
        if idx_razao is not None and idx_razao < len(campos):
            result["razao_social"] = campos[idx_razao].strip()

        # Extrair código CVM
        if idx_cod is not None and idx_cod < len(campos):
            result["cod_cvm"] = campos[idx_cod].strip()
