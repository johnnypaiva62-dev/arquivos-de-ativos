...
def descobrir_dados_fii(ticker: str) -> dict:
    """
    Busca CNPJ e Razão Social de um FII no cadastro da CVM.
    Retorna dict com cnpj, razao_social e cod_cvm.

    IMPORTANTE: A CVM unificou os cadastros. O arquivo antigo
    dados.cvm.gov.br/dados/FII/CAD/DADOS/cad_fii.csv não existe mais.
    Agora usamos: dados.cvm.gov.br/dados/FI/CAD/DADOS/cad_fi.csv
    """
    ...
    urls = [
        # 1. Cadastro unificado de FI (atual, ~17MB)
        "https://dados.cvm.gov.br/dados/FI/CAD/DADOS/cad_fi.csv",
        # 2. Cadastro antigo de FII (pode não existir mais, mas tentamos)
    ...
    for url in urls:
        ...
        # Parse CSV — delimitador é ;
        lines = resp.text.split("\n")
        if len(lines) < 2:
            continue

        # Encontrar colunas relevantes no header
        header = lines[0].split(";")
        header_clean = [h.strip().upper().replace('"', '') for h in header]

        # Mapear índices
        col_map = {}
        for i, h in enumerate(header_clean):
            if "CNPJ" in h and "cnpj" not in col_map:
                col_map["cnpj"] = i
            if any(x in h for x in ["DENOM", "RAZAO", "NOME_FUNDO", "FUNDO"]):
                if "nome" not in col_map:
                    col_map["nome"] = i
            if h in ("TP_FUNDO", "TIPO_FUNDO", "CLASSE"):
                col_map["tipo"] = i
            if "COD_CVM" in h or h == "CD_CVM":
                col_map["cod_cvm"] = i

        # Buscar o ticker nas linhas
        for linha in lines[1:]:
            if not linha.strip():
                continue

            # Buscar ticker na linha (pode estar em qualquer campo)
            if ticker_upper not in linha.upper():
                continue
            ...
            # Extrair CNPJ
            if "cnpj" in col_map and col_map["cnpj"] < len(campos_clean):
                cnpj_val = campos_clean[col_map["cnpj"]]
                # Aceitar tanto formatado quanto só números
                if re.match(r"\d{2}\.\d{3}\.\d{3}/\d{4}-\d{2}", cnpj_val):
                    result["cnpj"] = cnpj_val
                elif re.match(r"\d{14}", cnpj_val):
                    # Formatar CNPJ
                    c = cnpj_val
                    result["cnpj"] = f"{c[:2]}.{c[2:5]}.{c[5:8]}/{c[8:12]}-{c[12:14]}"

            # Fallback: procurar CNPJ em qualquer campo da linha
            if not result["cnpj"]:
            ...
            if "nome" in col_map and col_map["nome"] < len(campos_clean):
                result["razao_social"] = campos_clean[col_map["nome"]]

            # Extrair código CVM
            if "cod_cvm" in col_map and col_map["cod_cvm"] < len(campos_clean):
                result["cod_cvm"] = campos_clean[col_map["cod_cvm"]]

            if result["cnpj"]:
                return result

        # Se encontrou algo nesta URL, retornar
        if result["cnpj"]:
            return result
