...
def _descobrir_cnpj_classe(csvs_por_ano: dict, cnpj_fundo: str, ticker: str = "") -> list:
    ...
    Estratégia principal: encontrar um CSV que tenha TANTO CNPJ_Fundo 
    QUANTO CNPJ_Fundo_Classe, fazer match pelo CNPJ_Fundo, e pegar o _Classe.
    """
    ...
    for ano in sorted(csvs_por_ano.keys(), reverse=True):
        ...
        for nome_csv, rows in csvs.items():
            ...
            # Identificar colunas
            col_classe = None
            col_fundo = None  # CNPJ_Fundo (sem Classe)
            
            for c in colunas:
                cl = c.lower()
                if "cnpj" in cl and "classe" in cl:
                    col_classe = c
                elif cl in ("cnpj_fundo", "cnpj") or ("cnpj" in cl and "fundo" in cl and "classe" not in cl and "admin" not in cl):
                    col_fundo = c
            
            if not col_classe:
                continue
            
            # MELHOR ESTRATÉGIA: CSV tem CNPJ_Fundo E CNPJ_Fundo_Classe
            if col_fundo:
                for row in rows:
                    val = re.sub(r"\D", "", str(row.get(col_fundo, "")))
                    if val == cnpj_limpo:
                        cnpj_c = re.sub(r"\D", "", str(row.get(col_classe, "")))
                        if cnpj_c and cnpj_c != cnpj_limpo:
                            cnpjs_encontrados.add(cnpj_c)
            
            # FALLBACK: buscar ticker em qualquer valor da linha
            if ticker_upper and not cnpjs_encontrados:
                for row in rows:
                    for c in colunas:
                        val = str(row.get(c, "")).upper()
                        if ticker_upper in val or (ticker_base and len(ticker_base) >= 3 and ticker_base in val):
                            cnpj_c = re.sub(r"\D", "", str(row.get(col_classe, "")))
                            if cnpj_c and cnpj_c != cnpj_limpo:
                                cnpjs_encontrados.add(cnpj_c)
                            break
            
            # Se CNPJ do fundo É o CNPJ_Fundo_Classe (formato antigo)
            for row in rows[:20]:
        ...
        if cnpjs_encontrados:
            break  # Encontrou, não precisa de anos anteriores
