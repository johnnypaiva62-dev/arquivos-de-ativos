...
async def debug_trimestral_cnpj(ticker: str, ano: int = 2025):
    """
    Debug: diagnóstico focado no CSV resultado_contabil_financeiro.
    """
    ticker = ticker.upper().strip()
    dados_fii = descobrir_dados_fii(ticker)
    cnpj = dados_fii.get("cnpj", "")
    ...
    for nome_csv, rows in sorted(csvs.items()):
        ...
        # Info resumida para todos os CSVs
        filtrado = _filtrar_por_cnpj(rows, cnpj, cnpjs_classe)
        info = {
            "rows": len(rows),
            "match": len(filtrado),
        ...
        # Para resultado_contabil: info detalhada
        if "resultado_contabil" in nl:
            # Primeiros 5 CNPJs únicos no CSV
            col_cnpj = None
            ...
            cnpjs_no_csv = []
            if col_cnpj:
                cnpjs_unicos = list(set(str(r.get(col_cnpj, "")) for r in rows))[:10]
                cnpjs_no_csv = cnpjs_unicos
                
                # Testar match manual
                match_manual = 0
                for r in rows:
                    v = re.sub(r"\D", "", str(r.get(col_cnpj, "")))
                    if v == cnpj_limpo:
                        match_manual += 1
            
            info["col_cnpj"] = col_cnpj
            info["cnpjs_no_csv"] = cnpjs_no_csv
            info["nosso_cnpj"] = cnpj
            info["nosso_cnpj_limpo"] = cnpj_limpo
            info["match_manual"] = match_manual
            info["colunas"] = list(rows[0].keys())
            if filtrado:
                info["primeira_linha"] = filtrado[0]
            elif rows:
                info["primeira_linha_geral"] = {k: str(v)[:30] for k, v in rows[0].items()}
        
        resultado[nome_csv] = info
