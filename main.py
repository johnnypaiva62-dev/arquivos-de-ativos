...
def _filtrar_por_cnpj(rows: list, cnpj: str, cnpjs_classe: list = None) -> list:
    ...
    Aceita lista de CNPJs alternativos (classes CVM 175).
    """
    cnpj_limpo = re.sub(r"\D", "", cnpj)
...
def _descobrir_cnpj_classe(csvs_por_ano: dict, cnpj_fundo: str, ticker: str = "") -> list:
    ...
    Retorna lista de CNPJs únicos (pode haver mais de uma classe).
    """
    cnpj_limpo = re.sub(r"\D", "", cnpj_fundo)
    ticker_upper = ticker.upper().strip() if ticker else ""
    # Variantes do ticker para busca (ex: BLCA11 → BLCA, BRBLCACTF005)
    ticker_base = re.sub(r"\d+$", "", ticker_upper)  # BLCA
    ...
    for ano in sorted(csvs_por_ano.keys(), reverse=True):
        ...
        for nome_csv, rows in csvs.items():
            ...
            # Estratégia 1: buscar CNPJ em colunas não-Classe
            for col in cols_cnpj_outros:
                for row in rows:
                    ...
                    if val == cnpj_limpo:
                        ...
                        if cnpj_c:
                            cnpjs_encontrados.add(re.sub(r"\D", "", cnpj_c))
            
            # Estratégia 2: buscar por ticker em QUALQUER coluna
            if ticker_upper:
                for row in rows:
                    row_str = " ".join(str(v) for v in row.values()).upper()
                    if ticker_upper in row_str or (ticker_base and len(ticker_base) >= 3 and ticker_base in row_str):
                        cnpj_c = row.get(col_classe, "")
                        if cnpj_c:
                            cnpjs_encontrados.add(re.sub(r"\D", "", cnpj_c))
                            break  # Encontrou, não precisa buscar mais neste CSV
            
            # Estratégia 3: CNPJ do fundo pode SER o CNPJ_Fundo_Classe (formato antigo)
            for row in rows[:50]:
    ...
    # Remover o próprio CNPJ do fundo (já é buscado diretamente)
    cnpjs_encontrados.discard(cnpj_limpo)
...
async def cvm_contabilidade(
    ...
    _debug = {
        ...
        "csvs_encontrados": [],
        "cnpj_buscado": cnpj,
        "cnpjs_classe_descobertos": cnpjs_classe,
        "todos_csvs_estrutura": [],  # Debug: mostrar TODOS os CSVs e suas colunas CNPJ
    }

    # Debug: listar TODOS os CSVs e seus campos CNPJ
    for ano in sorted(trimestrais.keys()):
        ...
        for nome_csv, rows in sorted(csvs.items()):
            if not rows:
                continue
            colunas = list(rows[0].keys())
            cols_cnpj = {c: str(rows[0].get(c, ""))[:30] for c in colunas if "cnpj" in c.lower()}
            # Buscar colunas com ticker/codigo
            cols_codigo = {}
            for c in colunas:
                if any(x in c.lower() for x in ["codigo", "isin", "negociacao", "ticker", "sigla", "nome"]):
                    cols_codigo[c] = str(rows[0].get(c, ""))[:50]
            _debug["todos_csvs_estrutura"].append({
                "csv": nome_csv,
                "total_rows": len(rows),
                "cols_cnpj": cols_cnpj,
                "cols_codigo": cols_codigo,
            })
    ...
    for ano in sorted(trimestrais.keys()):
        ...
        for nome_csv, rows in csvs.items():
            ...
            _debug["csvs_encontrados"].append({
                "csv": nome_csv,
                "ano": ano,
                "total_rows": len(rows),
                "filtrado_ticker": len(dados),
            })
...
@app.get("/api/cvm/debug-trimestral-cnpj/{ticker}")
async def debug_trimestral_cnpj(ticker: str, ano: int = 2025):
    """
    Debug: mostra TODOS os CSVs do trimestral e quantos registros batem com o CNPJ do ticker.
    Resposta leve — só nomes de CSVs e contagens.
    """
    ticker = ticker.upper().strip()
    dados_fii = descobrir_dados_fii(ticker)
    cnpj = dados_fii.get("cnpj", "")
    ...
    for nome_csv, rows in sorted(csvs.items()):
        ...
        # Colunas CNPJ com exemplos
        cols_cnpj = {}
        for c in rows[0].keys():
            if "cnpj" in c.lower():
                vals_unicos = list(set(str(r.get(c, ""))[:25] for r in rows[:10]))
                cols_cnpj[c] = vals_unicos
        
        # Filtrar
        filtrado_direto = len([r for r in rows if any(
            re.sub(r"\D", "", str(r.get(c, ""))) == cnpj_limpo
            for c in r.keys() if "cnpj" in c.lower()
        )])
        filtrado_classe = len(_filtrar_por_cnpj(rows, cnpj, cnpjs_classe))
        
        resultado.append({
            "csv": nome_csv,
            "rows": len(rows),
            "match_cnpj_direto": filtrado_direto,
            "match_com_classe": filtrado_classe,
            "cols_cnpj": cols_cnpj,
        })
