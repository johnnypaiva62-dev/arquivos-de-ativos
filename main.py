...
def _extrair_composicao_ativo_passivo(row: dict) -> dict | None:
    ...
    mapeamento_ativos = {
        ...
        "Disponibilidades": "caixa_liquidez",
        ...
        "Obrigacoes_Securitizacao": "obrig_securitizacao",
        # Valores a receber
    ...
    mapeamento_passivos = {
        ...
        "Obrigacoes_Securitizacao_Passivo": "obrig_securitiz_passivo",
    }
    ...
    return {"ativos": ativos, "passivos": passivos}


def _extrair_complemento(row: dict) -> dict | None:
    """Extrai PL, cotistas e cotas do CSV complemento ou geral."""
    dados = {}
    for k, v in row.items():
        kl = k.lower()
        val = _extrair_float(v)
        ...
        elif k in ("NR_COTST", "Total_Cotistas", "Numero_Cotistas") or ("cotist" in kl and ("total" in kl or "nr" in kl or "numero" in kl)):
            dados["cotistas"] = val
        elif k in ("QT_COTAS", "NR_COTAS_EMITIDAS", "Quantidade_Cotas_Emitidas"):
            dados["cotas_emitidas"] = val
        elif ("cota" in kl and ("emitida" in kl or "qt" in kl or "quant" in kl) and "cotist" not in kl):
            if val > 1000:
                dados["cotas_emitidas"] = val
        elif k == "PF" or ("pessoa" in kl and "fisic" in kl):
            dados["cotistas_pf"] = val
        elif "amort" in kl:
            dados["amortizacao"] = val
...
async def cvm_evolucao_patrimonial(ticker: str):
    ...
    for ano in range(2016, ano_atual + 1):
        ...
        for nome_csv, rows in csvs_ordenados:
            ...
            for row in filtrado:
                ...
                if (is_complemento or is_geral) and snap["complemento"] is None:
                    snap["complemento"] = _extrair_complemento(row)
    ...
    # 3. Último disponível (se não já incluído)
