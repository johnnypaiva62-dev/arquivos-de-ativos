...
def buscar_fnet(ticker: str, cnpj: str = None, razao_social: str = None, max_docs: int = 20) -> list:
    ...
    Tenta múltiplas estratégias de busca:
      1. Por CNPJ (mais confiável)
      2. Por razão social
      3. Por ticker (código de negociação)
      4. Combinações
    """
    ...
    # 1. CNPJ (mais confiável — é o identificador único do fundo)
    if cnpj:
        estrategias.append({
            "nome": "CNPJ",
            "cnpj": cnpj,
    ...
    # 4. Ticker sozinho (menos confiável, mas tenta)
    estrategias.append({
        "nome": "Ticker",
        "cnpj": "",
        "razaoSocial": "",
    ...
    # 5. CNPJ sem formatação (só dígitos)
    if cnpj:
    ...
    for estrategia in estrategias:
        nome = estrategia.pop("nome")
        payload = {**base_payload, **estrategia}
...
def descobrir_dados_fii(ticker: str) -> dict:
    """
    Busca CNPJ e Razão Social de um FII usando múltiplas fontes.
    
    Estratégia (em ordem de velocidade):
      1. Investidor10 — scraping rápido, tem CNPJ na página
      2. Funds Explorer — idem
      3. Status Invest — idem
      4. CVM cad_fi.csv — arquivo grande (17MB), pode dar timeout
    """
    result = {"cnpj": None, "razao_social": None, "cod_cvm": None}
    ticker_upper = ticker.upper().strip()
    ticker_lower = ticker.lower().strip()

    # ── Método 1: Investidor10 ──────────────────────────────
    try:
        url = f"https://investidor10.com.br/fiis/{ticker_lower}/"
        resp = requests.get(url, timeout=15, headers={
            "User-Agent": BROWSER_HEADERS["User-Agent"],
        })
        if resp.ok:
            # CNPJ aparece no HTML da página
            cnpj_match = re.search(r"(\d{2}\.\d{3}\.\d{3}/\d{4}-\d{2})", resp.text)
            if cnpj_match:
                result["cnpj"] = cnpj_match.group(1)

            # Razão social / nome do fundo
            soup = BeautifulSoup(resp.text, "html.parser")
            # Tentar pegar o nome completo do fundo
            name_el = soup.find("h2") or soup.find("title")
            if name_el:
                nome_raw = name_el.get_text(strip=True)
                # Limpar - remover sufixos como "| Investidor10"
                nome = re.sub(r"\s*[\|–-]\s*(Investidor10|investidor10).*", "", nome_raw).strip()
                if nome and len(nome) > 3:
                    result["razao_social"] = nome
    ...
    # ── Método 2: Funds Explorer ────────────────────────────
    try:
        url = f"https://www.fundsexplorer.com.br/funds/{ticker_lower}"
        resp = requests.get(url, timeout=15, headers={
            "User-Agent": BROWSER_HEADERS["User-Agent"],
        })
        if resp.ok:
            ...
            soup = BeautifulSoup(resp.text, "html.parser")
            # Funds Explorer tem o nome do fundo em vários lugares
            for tag in soup.find_all(["h1", "h2", "span"]):
                text = tag.get_text(strip=True)
                if "fundo" in text.lower() and len(text) > 10:
                    result["razao_social"] = text
                    break
    ...
    # ── Método 3: Status Invest ─────────────────────────────
    try:
        url = f"https://statusinvest.com.br/fundos-imobiliarios/{ticker_lower}"
        resp = requests.get(url, timeout=15, headers={
            "User-Agent": BROWSER_HEADERS["User-Agent"],
        })
        if resp.ok:
            ...
            if name_el:
                ...
                nome = re.sub(r"\s*[\|–-]\s*(Status Invest|statusinvest).*", "", nome, flags=re.IGNORECASE).strip()
                if nome and len(nome) > 3:
                    result["razao_social"] = nome
    ...
    # ── Método 4: CVM cad_fi.csv (fallback, arquivo grande) ──
    try:
        cvm_url = "https://dados.cvm.gov.br/dados/FI/CAD/DADOS/cad_fi.csv"
        resp = requests.get(cvm_url, timeout=120, headers={
            "User-Agent": BROWSER_HEADERS["User-Agent"],
        }, stream=True)
        if resp.ok:
            # Ler linha por linha para não estourar memória
            text = resp.text
            lines = text.split("\n")
            for linha in lines[1:]:
                if ticker_upper in linha.upper():
                    campos = linha.split(";")
                    campos_clean = [c.strip().replace('"', '') for c in campos]
                    # Procurar CNPJ na linha
                    for campo in campos_clean:
                        if re.match(r"\d{2}\.\d{3}\.\d{3}/\d{4}-\d{2}$", campo):
                            result["cnpj"] = campo
                            break
                        if re.match(r"\d{14}$", campo):
                            c = campo
                            result["cnpj"] = f"{c[:2]}.{c[2:5]}.{c[5:8]}/{c[8:12]}-{c[12:14]}"
                            break
                    if not result["cnpj"]:
                        match = re.search(r"(\d{2}\.\d{3}\.\d{3}/\d{4}-\d{2})", linha)
                        if match:
                            result["cnpj"] = match.group(1)
                    # Pegar o campo mais longo como nome/razão social (heurística)
                    if not result["razao_social"]:
                        longest = max(campos_clean, key=len) if campos_clean else ""
                        if len(longest) > 10 and not re.match(r"[\d./\-]+$", longest):
                            result["razao_social"] = longest
                    if result["cnpj"]:
                        return result
    except Exception:
        pass
...
async def debug_ticker(ticker: str):
    ...
    if tipo == "fii":
        # Etapa 1: Buscar dados no cadastro CVM
        ...
        testes = {
            "por_cnpj": {"cnpj": dados_fii.get("cnpj", ""), "razaoSocial": "", "codigoNegociacao": ""},
            "por_razao_social": {"cnpj": "", "razaoSocial": dados_fii.get("razao_social", ""), "codigoNegociacao": ""},
            "por_ticker": {"cnpj": "", "razaoSocial": "", "codigoNegociacao": ticker},
            "cnpj_e_ticker": {"cnpj": dados_fii.get("cnpj", ""), "razaoSocial": "", "codigoNegociacao": ticker},
        }
