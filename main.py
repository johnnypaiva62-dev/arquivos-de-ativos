...
async def cvm_evolucao_patrimonial(ticker: str):
    ...
    amortizacoes = []
    amort_acumulada_pct = 0.0
    amort_acumulada_valor = 0.0
    
    for periodo in periodos_ordenados:
        snap = todos_snapshots[periodo]
        compl = snap.get("complemento") or {}
        amort_pct = compl.get("amortizacao_pct_mes", 0) or 0
        vp = compl.get("vp_cota")
        
        if amort_pct and amort_pct > 0:
            # Estimar valor da amortização: % × VP da cota na época
            # Se não tiver VP, usar o VP do mês anterior
            valor_estimado = None
            if vp and vp > 0:
                valor_estimado = round(amort_pct * vp * 100, 4)  # pct já vem decimal (0.005 = 0.5%)
                # Verificar se faz sentido (amort_pct pode já estar em %)
                if valor_estimado > vp:
                    # Provável que pct está em formato decimal (0.005 = 0.5%)
                    valor_estimado = round(amort_pct * vp, 4)
            
            amort_acumulada_pct += amort_pct
            if valor_estimado:
                amort_acumulada_valor += valor_estimado
            
            amortizacoes.append({
                "periodo": periodo,
                "percentual": amort_pct,
                "valor_estimado_por_cota": valor_estimado,
                "vp_cota_no_mes": vp,
                "amort_acumulada_pct": round(amort_acumulada_pct, 6),
    ...
    if amortizacoes:
        ...
        resumo["amort_acumulada_pct"] = round(amort_acumulada_pct, 6)
