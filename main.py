...
def _baixar_csv_cvm_fii(ano: int) -> dict:
    ...
    um dict com os DataFrames (como lista de dicts) indexados
    pelo nome do CSV.
    
    Arquivos dentro do ZIP (típicos):
      - inf_mensal_fii_geral_AAAA.csv
      - inf_mensal_fii_complemento_AAAA.csv  
      - inf_mensal_fii_ativo_passivo_AAAA.csv
    """
    ...
    try:
        ...
        return {"erro": f"Erro ao baixar ZIP da CVM: {str(e)}"}
    ...
    try:
        ...
        return {"erro": f"Erro ao processar ZIP: {str(e)}"}
...
def _filtrar_por_cnpj(rows: list, cnpj: str) -> list:
    """Filtra linhas por CNPJ (tenta ambas as colunas possíveis)."""
    cnpj_limpo = re.sub(r"\D", "", cnpj)
    resultado = []
    for row in rows:
        # Tentar ambas as colunas (CVM mudou o nome em 2025)
        row_cnpj = row.get("CNPJ_FUNDO") or row.get("CNPJ_FUNDO_CLASSE") or ""
...
async def cvm_informe_mensal(
    ...
    Dados incluem: patrimônio líquido, ativos, passivos, receitas, cotistas,
    rendimentos distribuídos, vacância, etc.
    ...
    if tipo != "fii":
        return {
            "ticker": ticker,
            "erro": "Informe mensal CVM disponível apenas para FIIs",
            "tipo": tipo,
        }
    ...
    if not cnpj:
        return {
            "ticker": ticker,
            "erro": f"CNPJ não encontrado para {ticker}",
            "cadastro": dados_fii,
        }
    ...
    # Baixar e filtrar CSVs
    csvs = _baixar_csv_cvm_fii(ano)
    if "erro" in csvs:
        # Tentar ano anterior como fallback
        csvs = _baixar_csv_cvm_fii(ano - 1)
        if "erro" in csvs:
            return {
                "ticker": ticker,
                "cnpj": cnpj,
    ...
    resultado = {
        ...
        "ano": ano,
        "fonte": f"https://dados.cvm.gov.br/dados/FII/DOC/INF_MENSAL/DADOS/inf_mensal_fii_{ano}.zip",
        "dados": {},
    }
    ...
    for nome_csv, rows in csvs.items():
        ...
        # Ordenar por data de competência (mais recente primeiro)
        for row in filtrado:
            row["_dt"] = row.get("DT_COMPTC", row.get("Data_Competencia", ""))
        ...
        # Determinar o tipo de CSV pelo nome
        nome_curto = nome_csv.replace(f"_{ano}", "").replace("inf_mensal_fii_", "").replace(".csv", "")
        resultado["dados"][nome_curto] = {
            "arquivo_csv": nome_csv,
            "total_registros": len(filtrado),
            "registros": filtrado,
        }

    resultado["total_csvs_encontrados"] = len(resultado["dados"])
    resultado["consultado_em"] = datetime.now().isoformat()
