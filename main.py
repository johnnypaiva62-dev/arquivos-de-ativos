...
def _descobrir_cnpj_classe(csvs_por_ano: dict, cnpj_fundo: str, ticker: str = "") -> list:
    ...
    for ano in sorted(csvs_por_ano.keys(), reverse=True):
        ...
        for nome_csv, rows in csvs.items():
            ...
            # Estratégia 2: buscar por ticker
            if ticker_upper:
                for c in colunas:
                    cl = c.lower()
                    if any(x in cl for x in ["codigo", "isin", "negociacao", "ticker", "sigla"]):
                        for row in rows:
                            val = str(row.get(c, "")).upper().strip()
                            if ticker_upper in val:
                                cnpj_c = row.get(col_classe, "")
                                if cnpj_c:
                                    cnpjs_encontrados.add(re.sub(r"\D", "", cnpj_c))
            
            # Estratégia 3: CNPJ no campo Fundo_Classe pode SER o do fundo (formato antigo)
            for row in rows[:50]:
