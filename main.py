...
def _extrair_contabilidade(rows: list, cnpj: str) -> list:
...
async def cvm_contabilidade(
    ...
    for ano in sorted(trimestrais.keys()):
        ...
        for nome_csv, rows in csvs.items():
            ...
            dados = _extrair_contabilidade(rows, cnpj)
            if not dados:
                continue
