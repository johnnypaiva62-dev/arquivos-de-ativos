    ...
    for ano in sorted(trimestrais.keys()):
        ...
        for nome_csv, rows in csvs.items():
            if not rows:
                continue
            nl = nome_csv.lower()
            # Filtrar CSVs relevantes para contabilidade/DRE
            csvs_contab = ["contabilidade", "rentabilidade_efetiva", "resultado_contabil", "resultado_financeiro", "balanco", "dre"]
            if not any(termo in nl for termo in csvs_contab):
                continue
            ...
            if "dre" in nl or "rentabilidade_efetiva" in nl or "resultado_contabil" in nl or "resultado_financeiro" in nl:
                for d in dados:
                    d["_fonte"] = "trimestral"
                    d["_csv"] = nome_csv
